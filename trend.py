...
try:
    ...
    res_indis = []
    for x in indis:
        res_indis.append(x[-1])
    id_indis = ['Bollinger Bands', 'Double Exponential Moving Average', 'Exponential Moving Average','Hilbert Transform - Instantaneous Trendline', 'Kaufman Adaptive Moving Average',
    ...
    for y in oscs:
        res_oscs.append(y[-1])
    id_oscs = ['Commodity Channel Index', 'MACD', 'Momentum', 'Rate of Change', 'Relative Strength Index', 'Stochastic', "Williams' %R"]
    val_oscs = [0, 0, 100, 0, 50, 50, -50]
    ...
    res_patts = []
    for z in patts:
        res_patts.append(z[-1])
    id_patts = ['Abandoned Baby', 'Belt-hold', 'Breakaway', 'Closing Marubozu', 'Concealing Baby Swallow', 'Counterattack', 'Dragonfly Doji', 'Engulfing Pattern', 'Up-gap Side-by-Side White Lines',
    ...
    error_trend = 'Fetching data failed due to an error on the yahoo finance server. Please come back later.'
