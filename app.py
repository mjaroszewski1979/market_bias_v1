...
from bias import adx, res_i, id_i, val_i, num_i, res_p, id_p, num_p, res_o, id_o, val_o, num_o, error1
from send_mail import send_mail
import threading

if adx != None:
    from strategies import top_index, nums, error2
else:
    error2 = 'Fetching data failed due to an error on the yahoo finance server. Please come back later.'
    top_index = None
    nums = None
...
def index():
    ...
    res_o=res_o, id_o=id_o, val_o=val_o, num_o=num_o, error = error1)
...
def strategies():
    return render_template('strategies_min.html', top_index=top_index, nums=nums, error=error2)
